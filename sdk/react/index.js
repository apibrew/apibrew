...
var model = require('@apibrew/client/model');
...
var ClientConsumer = ClientContext.Consumer;
function useClient() {
...
var ErrorHandlerContext = React__default.createContext(undefined);
var useErrorHandler = function useErrorHandler() {
  return React__default.useContext(ErrorHandlerContext);
};

function useRepository(entityInfo) {
  var client = useClient();
  return client.repository(entityInfo);
}

function useRecordByName(entityInfo, name, wi) {
  return useRecordBy(entityInfo, {
    name: name
  }, wi);
}
function useRecordBy(entityInfo, identifier, wi) {
  var repository = useRepository(entityInfo);
...
function useRecords(entityInfo, params, wi) {
  var repository = useRepository(entityInfo);
  ...
  var errorHandler = useErrorHandler();
  React.useEffect(function () {
    repository.list(params).then(function (response) {
...
function useResourceByName(resourceName, namespace) {
  if (namespace === void 0) {
    namespace = 'default';
  }
  var errorHandler = useErrorHandler();
  ...
  var client = useClient();
  React.useEffect(function () {
    client.getResourceByName(namespace, resourceName).then(setResource, errorHandler);
...
function useResourceById(id) {
  ...
  var client$1 = useClient();
  React.useEffect(function () {
    client$1.listResources().then(function (response) {
      var resource = response.find(function (resource) {
        return resource.id === id;
      });
      if (resource) {
        setResource(resource);
      } else {
        throw new client.ApiException(model.Code.RESOURCE_NOT_FOUND, "Resource with id " + id + " not found");
      }
    }, errorHandler);
  }, [id]);
  return resource;
...
function useTokenBody() {
  var client = useClient();
  var token = client.getTokenBody();
  if (!token) {
...
function useWatcher(entityInfo, filters) {
  var client$1 = useClient();
  var errorHandler = useErrorHandler();
  var _useState = React.useState(0),
    counter = _useState[0],
    setCounter = _useState[1];
  var eiw = JSON.stringify(entityInfo);
  ...
  React.useEffect(function () {
    var watchInternal = function watchInternal() {
      try {
        var _exit = false,
          _interrupt = false;
        return Promise.resolve(_for(function () {
          return !(_interrupt || _exit) && !controller.signal.aborted;
        }, void 0, function () {
          return _finallyRethrows(function () {
            return _catch(function () {
              console.log('Starting watch');
              return Promise.resolve(fetch(clientImpl.Urls.recordWatchUrl(client$1.getUrl(), entityInfo.restPath, filters), {
                method: 'GET',
                headers: _extends({
                  ...
                  return _catch(function () {
                    return Promise.resolve(reader.read()).then(function (_ref) {
                      ...
                      if (response.status !== 200) {
                        throw client.ApiException.fromError(event);
                      }
                      if (event.id === 'heartbeat-message') {} else {
                        triggerUpdate();
                      }
                    });
                  }, function (e) {
...
function useAxios() {
  console.log('we have axios', axios);
  return axios;
}

function prepareListRecordParamsFromQuerySearch(searchParams) {
  var result = {};
  ...
  if (searchParams.has('query')) {
    var query = JSON.parse(searchParams.get('query'));
    if (query) {
      result.query = query;
    }
  }
  ...
  if (searchParams.has('sorting')) {
    var sorting = JSON.parse(searchParams.get('sorting'));
    if (sorting) {
...
function prepareQuerySearchFromListRecordParams(params) {
  var result = {};
  if (params.limit) {
...
function useQueryListParams(defaultValues) {
  ...
  var _useState = React.useState(_extends({}, defaultValues, prepareListRecordParamsFromQuerySearch(searchParams))),
    listParams = _useState[0],
    setListParams = _useState[1];
  var setListParamsWrapped = function setListParamsWrapped(params) {
    setListParams(params);
    setSearchParams(prepareQuerySearchFromListRecordParams(params));
  };
  return [listParams, setListParamsWrapped];
}

var LocalStorageTokenStorage = /*#__PURE__*/function () {
  function LocalStorageTokenStorage(prefix) {
    this.prefix = prefix;
  }
  var _proto = LocalStorageTokenStorage.prototype;
  ...
  _proto.set = function set(name, token) {
    if (typeof window !== 'undefined') {
      var tokens = JSON.parse(window.localStorage.getItem(this.getKey()) || '{}');
      tokens[name] = token;
...
var cloudConnectionProvider = {
  ...
  getConnection: function getConnection(name) {
    var localStorageItem = localStorage.getItem("@apibrew/client/" + name + "/token");
    var connection = {
      name: name,
      title: name,
      serverConfig: {
    ...
    if (localStorageItem) {
      var tokenData = JSON.parse(localStorageItem);
...
exports.ErrorHandlerContext = ErrorHandlerContext;
exports.LocalStorageTokenStorage = LocalStorageTokenStorage;
exports.cloudConnectionProvider = cloudConnectionProvider;
exports.useAxios = useAxios;
exports.useClient = useClient;
exports.useErrorHandler = useErrorHandler;
exports.useQueryListParams = useQueryListParams;
exports.useRecordBy = useRecordBy;
exports.useRecordByName = useRecordByName;
exports.useRecords = useRecords;
exports.useRepository = useRepository;
exports.useResourceById = useResourceById;
exports.useResourceByName = useResourceByName;
exports.useTokenBody = useTokenBody;
