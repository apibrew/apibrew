select columns.column_name,
       ...
       exists(SELECT 1
              ...
              WHERE contype = 'p'
                and conname = ANY (key_column_usage.constraint_names)) as is_primary,
       exists(SELECT 1
              FROM pg_constraint c
              WHERE contype = 'u'
                and conname = ANY (key_column_usage.constraint_names)) as is_unique,
...
from information_schema.columns
         left join (select key_column_usage.table_name,
                           key_column_usage.table_schema,
                           key_column_usage.column_name,
                           array_agg(constraint_name) as constraint_names
                    from information_schema.key_column_usage
                    group by 1, 2, 3) key_column_usage
         ...
         left join LATERAL (SELECT nspname                                                       as table_schema,
                            ...
                            WHERE contype = 'f'
                              and conname = ANY (key_column_usage.constraint_names)
