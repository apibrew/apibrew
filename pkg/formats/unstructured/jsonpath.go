...
//    authors, err := jsonpath.JsonPathRead(bookstore, "$..authors")
//
// A jsonpath expression can be prepared to be reused multiple times :
//
//    allAuthors, err := jsonpath.JsonPathPrepare("$..authors")
//    ...
//    var bookstore interface{}
//    err = json.Unmarshal(data, &bookstore)
//    authors, err := allAuthors(bookstore)
//
// The type of the values returned by the `JsonPathRead` method or `JsonPathPrepare`
...
// JsonPathRead a path from a decoded JSON array or object ([]interface{} or unstructured.Unstructured)
...
func JsonPathRead(value interface{}, path string) (interface{}, error) {
	filter, err := JsonPathPrepare(path)
	if err != nil {
		return nil, err
	}
	return filter(value)
...
func (p *parser) prepareFilterFunc() JsonPathFilterFunc {
	actions := p.actions
